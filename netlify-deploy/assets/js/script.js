...
const STORE_THEME_KEY = "fashionStoreTheme";

function applyStoreTheme(theme){
  const isDark = theme === "dark";
  document.body.classList.toggle("store-dark", isDark);
  document.querySelectorAll(".theme-mode-btn").forEach((button) => {
    const icon = button.querySelector("i");
    const text = button.querySelector("span");
    button.setAttribute("aria-pressed", String(isDark));
    button.setAttribute("aria-label", isDark ? "Switch to white mode" : "Switch to dark mode");
    if(icon){ icon.className = isDark ? "fa-solid fa-sun" : "fa-solid fa-moon"; }
    if(text){ text.textContent = isDark ? "White" : "Dark"; }
  });
}

function initStoreThemeMode(){
  const savedTheme = localStorage.getItem(STORE_THEME_KEY) || "light";
  applyStoreTheme(savedTheme);
  document.querySelectorAll(".theme-mode-btn").forEach((button) => {
    if(button.dataset.themeBound === "true"){ return; }
    button.dataset.themeBound = "true";
    button.addEventListener("click", () => {
      const nextTheme = document.body.classList.contains("store-dark") ? "light" : "dark";
      localStorage.setItem(STORE_THEME_KEY, nextTheme);
...
function createProductCard(product){
  ...
  card.dataset.href = `product.html?id=${encodeURIComponent(product.id)}`;
  ...
  oldPrice.textContent = `MAD ${Math.max(Number(product.price || 0) * 2, Number(product.price || 0)).toFixed(0)}`;
  const colorCount = document.createElement("span");
  colorCount.className = "product-colors-count";
  const colorTotal = Math.max(1, Math.min(5, images.length || 1));
  colorCount.textContent = `${colorTotal} COLOUR${colorTotal === 1 ? "" : "S"}`;
  let select = null;
  if(sizes.length){
  ...
  syncStockButton();
  card.append(imageWrap, title, price, oldPrice, colorCount);
...
async function refreshCartStock(){
  try{
    ...
    cart.forEach((item) => {
      ...
      const stock = product ? stockForSize(product, item.size) : 0;
      if(Number(item.stock || 0) !== stock){ item.stock = stock; changed = true; }
    });
    if(changed){ saveCart(); }
  }catch(error){
    // Backend unreachable: keep the cached stock snapshot.
  }
}

async function renderCartPage(skipStockRefresh = false){
...
async function sendOrder(){
  ...
  try{
    ...
    if(paymentMethod === "card"){
      ...
      if(!payUrl){
        if(status){ status.textContent = `Order #${order.id} was saved, but card payment is not configured yet. Track it with your phone number.`; }
        if(submit){ submit.disabled = false; }
...
async function renderAdminCategoryList(){
  ...
  categories.forEach((category) => {
    ...
    button.addEventListener("click", async () => {
      if(confirm(`Delete category "${category.name}"? All of its products and their pictures will be deleted too.`)){ await apiDelete(`${API.categories}/${category.id}`); await refreshAdminData(); }
    });
...
async function renderAdminProductList(){
  ...
  products.forEach((product) => {
    ...
    button.textContent = "Delete";
    button.addEventListener("click", async () => {
      if(confirm(`Delete product "${product.name}"? Its pictures will be deleted too.`)){ await apiDelete(`${API.products}/${product.id}`); await refreshAdminData(); }
    });
    row.append(editButton, button);
    list.appendChild(row);
  });
